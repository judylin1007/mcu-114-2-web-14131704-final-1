...
export class CartService {
  ...
  getCartItems() { return this.cartItemsSignal.asReadonly(); }

  addToCart(product: Product, quantity: number = 1) {
    const currentItems = this.cartItemsSignal();
    const existingItem = currentItems.find(item => item.product.id === product.id);
    if (existingItem) {
      this.cartItemsSignal.set(currentItems.map(item =>
  ...
  updateQuantity(productId: number, quantity: number) {
    ...
    this.cartItemsSignal.set(this.cartItemsSignal().map(item =>
  ...
  clearCart() { this.cartItemsSignal.set([]); }

  totalItemsCount = computed(() => this.cartItemsSignal().reduce((sum, item) => sum + item.quantity, 0));
  totalCartPrice = computed(() => this.cartItemsSignal().reduce((sum, item) => sum + (item.product.price * item.quantity), 0));
}